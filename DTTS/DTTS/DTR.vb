            ...
            Case "Time Out"
                    ...
                    If (result = DialogResult.Yes) Then
                        Try
                            ...
                            Dim sql2 As String = "UPDATE Records SET Time_Out = @TIMEOUT WHERE User_Name = @USERNAME AND Date_Recorded = @DATERECORDED"
...
Start:
    ...
    Public Sub LogTimeIn()
        ...
        Do
            Try
                objComm = New OleDbCommand("INSERT INTO Records VALUES ('" & CInt(generatedLogID) & "','" & lnkViewUserRecords.Text.ToString.ToUpper & "','" & DateTimePicker1.Value.Date.ToString("MM/dd/yyyy") & "','" & mergedTimeIn & "','" & defOut & "','" & defIn & "','" & defOut & "','" & defRen & "')", objConn)
                objAdap = New OleDbDataAdapter(objComm)
                objDt = New DataTable
                objAdap.Fill(objDt)
                wasSuccessful = True
    ...
    Sub UpdateDataGrid(ByVal DisplayDialog As Boolean)
        ...
        objDt = New DataTable
        objComm = New OleDbCommand("SELECT Date_Recorded AS [Date], Time_In AS [Time In], Time_Out AS [Time Out] FROM Records WHERE User_Name= '" & lnkViewUserRecords.Text.ToString.ToUpper & "'", objConn)
        objAdap = New OleDbDataAdapter(objComm)
